...
def main(args):
    ...
    print('iiLoci', 'SpaceOccupied', 'Length', '%GC Content', sep='\t')
    spaceocc = iiloci['EffectiveLength'].sum()
    spaceoccperc = spaceocc / table_genomesize * 100
    occ = '{:.1f} Mb ({:.1f}%)'.format(spaceocc / 1000000, spaceoccperc)

    lenqnt = list(iiloci['Length'].quantile([.25, .5, .75]))
    lenqnt = [int(x) for x in lenqnt]
    gcqnt = list(iiloci['GCContent'].quantile([.25, .5, .75]))
    gcqnt = [float('{:.3f}'.format(x)) for x in gcqnt]
    print(len(iiloci), occ, lenqnt, gcqnt, sep='\t')
